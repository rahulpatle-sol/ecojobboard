...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (token && storedUser) {
      ...
      const routeMap = { TALENT: `/DashboardJobseeker/${userId}`, HR: '/DashboardHR', MENTOR: '/MentorDashboard' };
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 md:px-24">
        <div className="max-w-md w-full mx-auto">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="email" placeholder="Email Address" required className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3.5 focus:border-blue-500 outline-none transition" onChange={(e)=>setForm({...form, email: e.target.value})} />
            <input type="password" placeholder="Password" required className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3.5 focus:border-blue-500 outline-none transition" onChange={(e)=>setForm({...form, password: e.target.value})} />
          ...
          <p className="text-center text-slate-400 mt-8">Don't have an account? <Link to="/signup" className="text-blue-400 font-bold hover:underline">Sign up</Link></p>
        </div>
      </div>
